...
select count(*) as Numbers from customers
...
select DepartmentName , count(*) as COUNTED from employees
group by departmentName

-- 8

select Category , min(Price) as MINIMUM , max(Price) as MAXIMUM from products
group by category
...
select ProductID , avg(SaleAmount) as AVERAGE , sum(SaleAmount) as TOTAL from sales
group by ProductID

-- 12

select count(*) as EmployeeCOUNTED from employees
where DepartmentName = 'HR'

-- 13

select DepartmentName , min(Salary) as MINIMUM , max(salary) as Maximum from employees
group by departmentName

-- 14 

select departmentname , avg(salary) as Averagesalary from employees
group by departmentname

-- 15 

select DepartmentName , avg(salary) as Averagesalary , count(EmployeeID) as CountedEmployees from employees
group by DepartmentName
...
select Year(SaleDate) as SaleYEAR , sum(SaleAmount) as TOTAL from sales
...
Select DepartmentName , sum(salary) as TOTALSALARY , avg(salary) as averagesalary from employees
group by departmentname
having avg(salary) > 65000

-- 23

select CustomerID , sum(totalamount) as TOTAL from orders
group by customerID
having avg(totalamount) > 50

-- 24

select year(orderdate) as year, month(orderdate) as month, sum(totalamount) as total, count(distinct productid) as products
from orders
...
select year(orderdate) as year, min(quantity) as min_qty, max(quantity) as max_qty
from orders
