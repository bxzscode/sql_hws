...
-- easy lvl
--1
select s.SupplierID , p.productname from products p
cross join suppliers s

--2
select departments.DepartmentID , Employees.[name] from Departments
...
select suppliers.supplierID , Products.Productname from products
left join suppliers
...
--9
select o.orderid , p.paymentmethod from payments p
join orders o
...
--18
select p.productname , c.categoryname , c.categoryID from products p
join categories c
on c.categoryid = p.category
where c.categoryid = 1 or c.categoryid = 3

--19
select c.firstname , c.customerID from customers c
join sales s
on c.customerid = s.customerid
where country like 'USA'

--20
select o.orderid , c.customerid , c.firstname from orders o
join customers c
...
where o.quantity * p.price <> ps.amount
