...
substring(name , charindex(',' , name) + 1 , len(name)) as lastname from TestMultipleColumns
-- 2
select * from testpercent
where strs like '%/%%'
escape '/'
--3
select id, substring(vals, 1, charindex('.', vals + '.') - 1) as firstpart, 
substring(vals, charindex('.', vals + '.') + 1, charindex('.', vals + '..', charindex('.', vals + '.') + 1) - charindex('.', vals + '.') - 1) as secondpart, 
substring(vals, charindex('.', vals + '..', charindex('.', vals + '.') + 1) + 1, len(vals)) as thirdpart
from splitter;
--4
select * from testDots
where len(vals) - len(replace(vals , '.' , '')) >= 2
--5 
select * , (len(texts) - len(replace(texts , ' ' , ''))) as lenth from CountSpaces
--6 
select e.first_name, e.salary, m.first_name as manager_name, m.salary as manager_salary from employees e
join employees m
...
--medium level 
-- 1
select w1.id , w.id , w1.Temperature , w.Temperature from weather w
join weather w1
...
--3 
select substring(fruit_list, charindex(',', fruit_list, charindex(',', fruit_list) + 1) + 1, charindex(',', fruit_list, charindex(',', fruit_list, charindex(',', fruit_list) + 1) + 1) 
 - charindex(',', fruit_list, charindex(',', fruit_list) + 1) - 1) as third_fruit
from fruits
--4
select *,case when datediff(year, hire_date, getdate()) < 1 then 'new hire'
...
where a.event_date = (select min(event_date) from activity where a.player_id = player_id)

--4
select 'rtcfvty34redt' as original_string, string_agg(case when substring('rtcfvty34redt', n, 1) like '[0-9]' then substring('rtcfvty34redt', n, 1) end, '') as numbers,
string_agg(case when substring('rtcfvty34redt', n, 1) like '[A-Za-z]' then substring('rtcfvty34redt', n, 1) end, '') as letters
from (select top (len('rtcfvty34redt'))
