...
-- easy level
-- 1
select concat(employee_id , '-' , first_name , ' ' , last_name) as Comb from employees
--2
update employees
set phone_number = replace (Phone_number , '124' , '999')
where PHONE_NUMBER like '%124%'
--3
select First_name , len(first_name) as Lenth from employees
where first_name like '[AJM]%'
order by first_name 
--4
select manager_id , sum(salary) as [sum] from employees
group by manager_id
--5
select year1 , (select max(v) from (values(Max1), (Max2), (Max3)) as x(v)) from testmax
--6 
select * from cinema
where id % 2 = 1  and description <> 'boring'
--7
select * from singleorder order by nullif(id, 0) desc
--8
select coalesce(ssn , passportid , itin) as NOTNULL from person
...
--5
select count(employee_id) AS counted , department_id , sum(case when datediff(Year , hire_date , getdate()) > 3 then 1 else 0 end) * 100 / count(*) as total_years from employees
group by department_ID

--Hard level
--1 
select * , sum(grade) over(order by studentid) as DID from students
--2 
select * from student
select s.StudentName , s1.StudentName , s.Birthday from student s
join student s1
on s.StudentName <> s1.StudentName and s.Birthday = s1.Birthday
--3 
select 
...
--4
select string_agg(case when substring(s, n, 1) like '[A-Z]' then substring(s, n, 1) end, '') as Uppercase,
string_agg(case when substring(s, n, 1) like '[a-z]' then substring(s, n, 1) end, '') as Lowercase,
string_agg(case when substring(s, n, 1) like '[0-9]' then substring(s, n, 1) end, '') as Numbers,
string_agg(case when substring(s, n, 1) not like '[A-Za-z0-9]' then substring(s, n, 1) end, '') as Others
from (select 'tf56sd#%OqH' as s) base
join (select top (len('tf56sd#%OqH')) row_number() over(order by (select null)) as n 
