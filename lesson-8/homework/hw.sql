...
select Category , count(Productname) as TOTAL from Products
group by Category
...
Select max(quantity) as [MAX] , min(Quantity) as [MIN] from orders

-- MEDIUM LVL
-- 10

select distinct CustomerID from orders
where OrderDate >= '2023-01-01' and orderdate < '2023-02-01' and CustomerID not in(select CustomerID from Invoices);
...
-- 13
select year(OrderDate) as [YEAR] , avg(totalamount) as Average from orders
...
into city_population_original 
from population_each_year 
unpivot (population for year in ([2012],[2013])) as unpvt;
select * from city_population_original
...
unpivot (population for district_name in ([bektemir],[chilonzor],[yakkasaroy])) as unpvt;
select * from city_population_original
