...
--3 
select d.departmentname , max(e.salary) as [MAX] from employees e
join departments d
on e.departmentid = d.departmentid
...
--5
select c.firstname , count(o.orderid) as [total] from customers c
join orders o
on c.customerid = o.customerid
...
--10
select p.productname , sum(s.saleamount) as TOTALAMOUNT  from products p
join sales s
on p.productid = s.productid
...
join departments d
on e.DepartmentID = d.DepartmentID
where d.departmentname = 'Human resources' and e.salary > 60000   -- there is no HR department in the table as question asked for it. I chose Human Resources
...
--16 
select c.categoryname , count(p.productname) as ProductCount from products p
