...
(40, 'Liam', 'Harris', 'liamh@live.com', '555-0123', '2828 Birch St', 'Richmond', 'VA', '23220','USA');
 ...
 3. CREATE TABLE Products_Discounted (
    ProductID INT PRIMARY KEY,
    ProductName VARCHAR(100),
    Price DECIMAL(10, 2),
    Category VARCHAR(50),
...
7. select ProductName , Stockquantity , iif(StockQuantity > 100 , 'Yes' , 'No') as Checker from products_discounted
...
10. select ProductName , Price , iif(Price > 1000 , 'Expensive' , 'Affordable') as Opinion from products
...
12. update employees
set Salary = case 
when DepartmentName = 'HR' or EmployeeID = 5 then Salary + Salary * 0.1
else Salary
end;
select * from employees
